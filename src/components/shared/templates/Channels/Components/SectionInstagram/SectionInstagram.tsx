...
import { setAccountIntagram } from '../../../../../../redux/slices/channels/account-instagram';
import {
  useAppDispatch,
  useAppSelector,
...
export const SectionComponentInstagram: FC<IPropsInstagram> = ({
  ...
  const { dataInfoIntagram } = useAppSelector(
  ...
  const handleHasPageInstagram = async () => {
    try {
      ...
      if (response.success === false) {
        ...
        dispatch(setAccountIntagram(response));
      }
    } catch (err) {
      showAlert?.addToast({
        alert: Toast.ERROR,
  ...
  const handleSendData = async () => {
    try {
      if (dataInfoIntagram && isActiveCheckbox && isSectionComponent === 2) {
        const response = await readPageInstagram(dataInfoIntagram);
        showAlert?.addToast({
          alert: Toast.SUCCESS,
          title: '¡Perfecto!',
        ...
        <div>
          ...
          {isSectionComponent === 2 ? (
            <LinkInstagramAccount
              dataInfoIntagram={dataInfoIntagram}
